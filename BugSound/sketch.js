let bgImage, score, testBug, walking, squished, time, gameState, starTime, timerIsDone, bugGroup, squishSound;
let bugImages = [];
let song;
...
function draw() {
  ...
  if (gameState === "start"){
    ...
    if (timer() % 10 == 0){
      song.rate(1 + (timer() % startTime / 30))
    }
    teleport();
